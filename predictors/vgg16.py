...
sym, arg_params, aux_params = mx.model.load_checkpoint(projectPath + 'params/vgg16', 0)
...
def predictionAndFeature(imgPath):
    ...
    symbols = []
    symbols.append(all_layers['flatten_0_output']) #An often used layer for feature extraction is the one before the last fully connected layer.
    ...
    symbols.append(all_layers['fc8_output'])
...
def getFeatureMap_resized(imgPath):
    ...
    all_layers = sym.get_internals()
    symbols = []
    symbols.append(all_layers['pool5_output'])
    symbols = mx.symbol.Group(symbols)
